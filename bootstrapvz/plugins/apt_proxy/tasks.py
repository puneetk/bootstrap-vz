...
class CheckAptProxy(Task):
    ...
    def run(cls, info):
        ...
        proxy_url = 'http://{address}:{port}'.format(
            address=proxy_address, port=proxy_port)
        try:
            urllib2.urlopen(proxy_url, timeout=5)
        except Exception as e:
            # Default response from `apt-cacher-ng`
            if isinstance(e, urllib2.HTTPError) and e.code in [
                    404, 406
            ] and e.msg == 'Usage Information':
                pass
            else:
                import logging
                log = logging.getLogger(__name__)
                log.warning(
                    'The APT proxy server couldn\'t be reached. `apt-get\' commands may fail.'
                )
    ...
    def run(cls, info):
        ...
        with open(proxy_path, 'w') as proxy_file:
            proxy_file.write(
                ...
                .format(
                    auth=proxy_auth, address=proxy_address, port=proxy_port))
