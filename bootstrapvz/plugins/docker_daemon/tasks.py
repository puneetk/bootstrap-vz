...
class AddDockerDeps(Task):
    ...
    DOCKER_DEPS = [
        'aufs-tools', 'btrfs-tools', 'git', 'iptables', 'procps', 'xz-utils',
        'ca-certificates'
    ]
...
class AddDockerBinary(Task):
    ...
    @classmethod
    def run(cls, info):
        docker_version = info.manifest.plugins['docker_daemon'].get(
            'version', False)
...
class AddDockerInit(Task):
    ...
    def run(cls, info):
        ...
        if docker_opts:
            sed_i(default_dest, r'^#*DOCKER_OPTS=.*$',
                  'DOCKER_OPTS="%s"' % docker_opts)
