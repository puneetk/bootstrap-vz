...
def modify_path(info, path, entry):
    ...
    if 'permissions' in entry:
        ...
        chmod_command = [
            'chroot', info.root, 'chmod',
            str(entry['permissions']), path
        ]
...
class MkdirCommand(Task):
    ...
    def run(cls, info):
        ...
        for dir_entry in info.manifest.plugins['file_copy']['mkdirs']:
            mkdir_command = [
                'chroot', info.root, 'mkdir', '-p', dir_entry['dir']
            ]
...
class ValidateFiles(Task):
    ...
    def run(cls, info):
        ...
        for i, file_entry in enumerate(
                info.manifest.plugins['file_copy']['files']):
            if not os.path.exists(
                    rel_path(info.manifest.path, file_entry['src'])):
                msg = 'The source file %s does not exist.' % file_entry['src']
                info.manifest.validation_error(
                    msg, ['plugins', 'file_copy', 'files', i])
...
class FileCopyCommand(Task):
    ...
    def run(cls, info):
        ...
        for file_entry in info.manifest.plugins['file_copy']['files']:
            ...
            final_destination = os.path.normpath("%s/%s" % (info.root,
                                                            file_entry['dst']))
