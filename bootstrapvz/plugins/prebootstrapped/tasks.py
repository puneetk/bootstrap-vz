...
class Snapshot(Task):
    ...
    predecessors = [
        packages.InstallPackages, guest_additions.InstallGuestAdditions
    ]
...
class CreateFromSnapshot(Task):
    ...
    def run(cls, info):
        ...
        ebs_volume = info._ec2['connection'].create_volume(
            info.volume.size.bytes.get_qty_in('GiB'),
...
class CopyImage(Task):
    ...
    @classmethod
    def run(cls, info):
        loopback_backup_name = 'volume-{id}.{ext}.backup'.format(
            id=info.run_id, ext=info.volume.extension)
        destination = os.path.join(info.manifest.bootstrapper['workspace'],
                                   loopback_backup_name)
...
class CreateFromImage(Task):
    ...
    @classmethod
    def run(cls, info):
        info.volume.image_path = os.path.join(
            info.workspace, 'volume.' + info.volume.extension)
        loopback_backup_path = info.manifest.plugins['prebootstrapped'][
            'image']
...
class CopyFolder(Task):
    ...
    @classmethod
    def run(cls, info):
        folder_backup_name = '{id}.{ext}.backup'.format(
            id=info.run_id, ext=info.volume.extension)
        destination = os.path.join(info.manifest.bootstrapper['workspace'],
                                   folder_backup_name)
...
class CreateFromFolder(Task):
    ...
    def run(cls, info):
        ...
        log_check_call([
            'cp', '-a', info.manifest.plugins['prebootstrapped']['folder'],
            info.root
        ])
