...
class AMIName(Task):
    ...
    def run(cls, info):
        ...
        ami_description = info.manifest.provider['description'].format(
            **info.manifest_vars)

        images = info._ec2['connection'].describe_images(
            Owners=['self'])['Images']
        for image in images:
            if 'Name' in image and ami_name == image['Name']:
                msg = 'An image by the name {ami_name} already exists.'.format(
                    ami_name=ami_name)
                raise TaskError(msg)
...
class BundleImage(Task):
    ...
    def run(cls, info):
        ...
        log_check_call([
            ...
            '--destination', info._ec2['bundle_path'], '--prefix',
            info._ec2['ami_name']
        ])
...
class UploadImage(Task):
    ...
    @classmethod
    def run(cls, info):
        manifest_file = os.path.join(info._ec2['bundle_path'],
                                     info._ec2['ami_name'] + '.manifest.xml')
        ...
        info._ec2[
            'manifest_location'] = info.manifest.provider['bucket'] + '/' + info._ec2['ami_name'] + '.manifest.xml'
        log_check_call([
            'euca-upload-bundle', '--bucket', info.manifest.provider['bucket'],
